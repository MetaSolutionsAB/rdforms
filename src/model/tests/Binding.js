...
var setup = function () {
  ...
  this.s1 = this.graph.create(
    rdfjson.tests.uris[0],
    rdfjson.tests.predicates[0]
  );
  this.s2 = this.graph.create(
    rdfjson.tests.uris[0],
    rdfjson.tests.predicates[1],
    { type: 'literal', value: 'Hepp' }
  );
...
doh.register('Binding-creation', [
  ...
  },
  {
    ...
    setUp: setup,
    runTest: function () {
      var b1 = new rdforms.model.ValueBinding({
        item: this.t1.getRoot().getChildren()[1],
      });
...
var setup2 = function () {
  ...
  this.b2 = new rdforms.model.ValueBinding({
    item: this.t1.getRoot().getChildren()[1],
    statement: this.s2,
  });
};
...
var setup4 = function () {
  ...
  this.s3 = this.graph.create(
    '_:person',
    'http://xmlns.com/foaf/0.1/firstname',
    { type: 'literal', value: 'Anna' }
  );
  this.s4 = this.graph.create('re1', 'http://purl.org/dc/elements/1.1/title', {
    type: 'literal',
    value: 'Some title',
  });
  this.is = new rdforms.template.ItemStore();
  this.t1 = this.is.createTemplate(rdforms.template.tests.template1);
  this.b1 = new rdforms.model.GroupBinding({ item: this.t1.getRoot() });
  this.b2 = new rdforms.model.GroupBinding({
    item: this.t1.getRoot().getChildren()[0],
    statement: this.s1,
    constraints: [this.s2],
  });
  this.b3 = new rdforms.model.ValueBinding({
    item: this.t1.getRoot().getChildren()[0].getChildren()[0],
    statement: this.s3,
  });
  this.b4 = new rdforms.model.ValueBinding({
    item: this.t1.getRoot().getChildren()[1],
    statement: this.s4,
  });
  this.b1.addChildBinding(this.b2);
...
var setup6 = function () {
  ...
  this.t1 = this.is.createTemplate(rdforms.template.tests.template1);
  this.b1 = new rdforms.model.GroupBinding({ item: this.t1.getRoot() });
  this.b2 = new rdforms.model.PropertyGroupBinding({
    item: this.t1.getRoot().getChildren()[4],
    statement: this.s1,
    constraints: [this.s2],
  });
  this.b3 = new rdforms.model.ValueBinding({
    item: this.t1.getRoot().getChildren()[4].getChildren()[1].getChildren()[0],
    statement: this.s3,
  });
  this.b4 = new rdforms.model.ValueBinding({
    item: this.t1.getRoot().getChildren()[1],
...
doh.register('Binding-assertions', [
  ...
  },
  {
    ...
    setUp: setup4,
    runTest: function () {
      this.s5 = this.graph.create(
        '_:person',
        'http://xmlns.com/foaf/0.1/surame',
        { type: 'literal', value: 'Wilder' }
      );
      this.b5 = new rdforms.model.ValueBinding({
        item: this.t1.getRoot().getChildren()[0].getChildren()[1],
        statement: this.s5,
      });
