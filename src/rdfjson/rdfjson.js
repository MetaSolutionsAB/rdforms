...
define([], function() {
	...
	//Three helper methods, from dojo.
	rdfjson.isObject = function(it){
 		return it !== undefined &&
			(it === null || typeof it == "object" || lang.isArray(it) || lang.isFunction(it)); // Boolean
	};
	rdfjson.isString = function(it){
		return (typeof it == "string" || it instanceof String); // Boolean
    };

    rdfjson.isArray = function(it){
	...
	rdfjson.checkForWrongArgs = function(args) {
		if (!rdfjson.isObject(args[0])) {
			throw "Graph is not a object.";
		} else if (!rdfjson.isString(args[1])) {
			throw "Subject is not a string.";		
		} else if (!rdfjson.isString(args[2])) {
			throw "Predicate is not a string.";		
		} else if (!rdfjson.isObject(args[3])) {
	...
	rdfjson.objectEquals =  function(o1, o2) {
		if (rdfjson.isString(o1)) {
			if (rdfjson.isString(o2)) {
				return o1 === o2;
			} else {
				return o1 === o2.value;
			}
		} else if (rdfjson.isString(o2)) {
			return o1.value === o2;
		} else {
