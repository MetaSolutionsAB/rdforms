...
define(["dojo/_base/declare", 
	...
	"dijit/TitlePane", //For template
	"rdfjson/Graph",
	"rforms/model/Engine",
	"rforms/template/ItemStore",
	"rforms/view/ValidationPresenter",
	"dojo/text!./ValidatorTemplate.html"
], function(declare, lang, array, window, json, domClass, construct, attr, style, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Dialog, Button, SimpleTextarea, TitlePane, Graph, Engine, ItemStore, ValidationPresenter, template) {
	...
	_validateJSON: function() {
		...
		attr.set(this._jsonNode, "innerHTML", "Valid JSON");
	    } catch (e) {
		domClass.toggle(this._jsonNode, "error", true);
		attr.set(this._jsonNode, "innerHTML", "Invalid JSON: "+e);
	    }
	},
	_validateRDFJSON: function() {
		...
		construct.create("div", {"innerHTML": "Valid RDF/JSON, "+this._graph.report.nr+" statements found."}, this._rdfjsonNode);
		//checkInstances(graph, "http://xmlns.com/foaf/0.1/Document", ttemplate);
	    } catch (e) {
		domClass.toggle(this._rdfjsonNode, "error", true);
		construct.create("div", {"innerHTML": "Invalid RDF/JSON, although "+e.nr+" valid statements found."}, this._rdfjsonNode);
