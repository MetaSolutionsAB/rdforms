...
doh.register("Engine create test", [
  { name: "Creating direct value",
	setUp: setup2,
	runTest: function() {
		var titleBinding = rforms.model.create(this.root, this.template.getRoot().getChildren()[1]);
  ...
  { name: "Creating group with direct values",
	setUp: setup2,
	runTest: function() {
		var groupBinding = rforms.model.create(this.root, this.template.getRoot().getChildren()[0]);
		doh.t(groupBinding.getParent() === this.root);
		doh.t(groupBinding.isValid() === false);
		doh.t(this.graph.find().length === 0);
		var valueBinding = rforms.model.create(groupBinding, this.template.getRoot().getChildren()[0].getChildren()[0]);
		doh.t(groupBinding.isValid() === false);
